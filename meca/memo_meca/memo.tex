...
\section{Déplacement $\rightarrow$ Vitesse $\rightarrow$ Accélération} % {{{1

$$\V{v}(M/\R) = \left(\frac{\D\V{OM}}{\D t}\right)_{/\B}$$
$$\V{\Gamma}(M/\R) = \left(\frac{\D\V{v}(M/\R)}{\D t}\right)_{/\B}$$

\section{Mouvement rectiligne uniforme} % {{{1

$$x(t) = v_0t + x_0$$

\section{Mouvement rectiligne uniformément varié} % {{{1

$$x(t) = \frac{1}{2}\gamma_0t + v_0t + x_0$$

\section{Point de rebroussement cinématique} % {{{1

On cherche le temps $t_1$ pour avoir $x(t)$ minimum :

$$t_1 = -\frac{v_0}{\gamma_0}$$

\section{Vecteur rotation instantannée} % {{{1
...
\section{Eléments de réduction d'un torseur} % {{{1
...
\section{Glisseur dynamique} % {{{1

$$\V{d}(A/\R) = m\V{\Gamma}(A/\R) = \left[A, m\V{\Gamma}(A/\R)\right]$$

\section{Principe fondamental de la Dynamique} % {{{1
